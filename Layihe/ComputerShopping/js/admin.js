...
function showList() {
   ...
   showListString="";
    ...
    for(var i=0;i<=kategoriya.length-1;i++){
     ...
     showListString+="<button class='list-group-item' onclick=select("+kategoriya[i].id+","+i+")>"+kategoriya[i].name+"</button>"
...
kategoriyaInput.value="";
 ...
 function select(id,i){
  ...
  setTimeout(function(){ 
   ...
   buttons.innerHTML=" <button onclick='redakte("+i+")' class='btn btn-warning'>Redakte et</button>"+
"<button type='reset' class='btn btn-dark' onvalue= 'Reset'> Temizle</button>"+
"<button onclick='deleteK("+i+")' class='btn btn-danger'>Sil</button>"
+"<button onclick='btne("+i+")' class='btn btn-info'>Secili olani legv et</button>"
   }, 50);
...
function btne(){
   buttons.innerHTML=" <button onclick='newKategoriya()' class='btn btn-success'>Elave et</button>"+
