...
public interface ComputerDao extends JpaRepository<Computer,Integer> {
    ...
    @Query(value = "select * from computer where username like %?1% or description like %?1% or price like %?1% or marka like %?1% or model like %?1% or yaddastipi like %?1%  "+
    "or emeliyyatsistemi like %?1%  or gpu like %?1%   or cpu like %?1% or yaddas like %?1%  or ram like %?1%       limit ?2,?3" ,nativeQuery = true)
